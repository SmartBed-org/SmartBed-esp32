...
void loop()
{
    ...
    if (Firebase.ready() && (millis() - dataMillis > 60000 || dataMillis == 0))
    {
        ...
        if (!taskCompleted)
        {
            ...
            content.set("fields/Singapore/mapValue/fields/time_zone/stringValue", "eight");
        ...
        if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), mask.c_str()))
        {
            ...
            String room = output.substring(room_index+29,room_index+30);
            Serial.printf("ok\n%s\n\n", room);

            // get bed number
            int bed_index = output.indexOf("bed number");
            String bed = output.substring(bed_index+28,bed_index+29);
            Serial.printf("ok\n%s\n\n", bed);

            Serial.println("R"+room+"B"+bed);
        }
        else
            Serial.println(fbdo.errorReason());
    }
